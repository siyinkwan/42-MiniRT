...
static void	compute_uv_sphere_plane(t_hit *t, double *u, double *v)
{
	t_vec3	d;
	t_vec3	local_u;
	t_vec3	local_v;
...
static void	compute_uv_cylinder_cone(t_hit *t, double *u, double *v)
{
	t_vec3	local_pos;
	t_vec3	radial;
	t_vec3	d;
...
void	compute_uv(t_hit *t, double *u, double *v)
{
	...
	if (t->object->type == OBJECT_SPHERE || t->object->type == OBJECT_PLANE)
		compute_uv_sphere_plane(t, u, v);
	else if (t->object->type == OBJECT_CYLINDER
		|| t->object->type == OBJECT_CONE)
		compute_uv_cylinder_cone(t, u, v);
	else
	{
