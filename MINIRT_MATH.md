...
```
1. Camera Setup → Build coordinate system (R, U, F)
2. For each pixel (x, y):
   a. Generate Ray → Calculate viewport position, create ray
   b. Find Intersection → Test all objects, get closest hit
   c. If hit:
      i.   Apply Bump Mapping → Perturb normal if enabled
      ii.  Calculate Ambient → Base illumination
      iii. For each light:
           - Shadow Test → Skip if in shadow
           - Calculate Diffuse → Lambertian reflection
           - Calculate Specular → Phong highlights
      iv.  Sum all components → Final color
   d. Else: Use background color
   e. Clamp & convert to RGB → Draw pixel
```
